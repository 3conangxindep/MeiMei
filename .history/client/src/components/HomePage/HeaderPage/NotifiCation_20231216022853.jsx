...
const Notification = ({ onClose }) => {
    ...
    return (
        <div className='notification-container absolute right-3 py-2.5 px-3.5 bg-gray-100 border border-solid border-gray-300 rounded-md shadow-md shadow-green-300 z-20'>
            ...
            <div className='mt-5'>
                {newNotification.map((e, index) => (
                    <NotificationItem key={`newNotification-${index}`} notification={e} setImg={setImg} />
                ))}
                {notification.map((e, index) => (
                    <NotificationItem key={`notification-${index}`} notification={e} setImg={setImg} />
                ))}
            </div>
        </div>
    );
