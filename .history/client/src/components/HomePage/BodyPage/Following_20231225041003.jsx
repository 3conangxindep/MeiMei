...
const Following = () => {
  ...
  //   setIsSaved(!isSaved);
  // };
  const handleStarClick = (event, id_card, index) => {
    event.preventDefault();
    //event.stopPropagation();

    fetch(`http://${API_BASE_URL}:8000/api/contact/like/${id_card}/${index}`, {
      method: 'PUT',
    })
      .then((response) => response.json())
      .then((responseData) => {
  ...
  return (
    <div>
      <div className='border-box relative w-full h-screen p-2.5 flex flex-col'>
        ...
        {data.map((e,index) => (
          <Link key={e.contact_id} to={`/InformationPage/${id_card}/${e.contact_id}`} className='list-none transition duration-200 cursor-pointer hover:bg-gray-200 hover:border-gray-200 hover:border hover:rounded-md'>
            {/* {setIsSaved(e.like)} */}
            {/* item-container */}
            <div className='relative flex items-center justify-between w-full h-16 pl-1.5 bg-gray-200 border-b-2 rounded-md border-b-solid border-b-gray-400'>
              ...
              <div className='relative w-56 h-4/5 py-3 px-1.5 flex items-center justify-between border border-solid border-gray-300 rounded-md bg-gray-300'>
                ...
                <div>
                  ...
                  <div className={index} onClick={(event) => handleStarClick(event, id_card, index)}>
                    {e.like ? (
                      <img
