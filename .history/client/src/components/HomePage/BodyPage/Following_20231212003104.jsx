    ...
    <div>
      <div className='border-box relative w-full h-screen p-2.5 flex flex-col'>
        <li className='list-none transition duration-200 cursor-pointer hover:bg-gray-200 hover:border-gray-200 hover:border hover:rounded-md'>
          {data.map((e) => (
            <div key={e.contact_id} className='relative flex items-center justify-between w-full h-16 pl-1.5 bg-gray-200 border-b-2 rounded-md border-b-solid border-b-gray-400'>
              ...
              <div className='absolute bottom-0 text-gray-500 right-2'>{e.time}</div>
            </div>
          ))}
        </li>
      </div>
      <p>
        {data.map((e) => {
          if (e.id_card === id_card) {
            return (
              <p key={e.contact_id}>
                Name: {e.name}, Mail: {e.mail}, Time: {e.time}
              </p>
            );
          }
