...
const NotifiCation = ({ onClose }) => {
    ...
    return (
        ...
        <div className='absolute right-3 py-2.5 px-3.5 bg-gray-100 border border-solid border-gray-300 rounded-md shadow-md shadow-green-300 z-20'>
            ...
            <div className='mt-5'>
                {/* Mapping over the 'notification' array to generate notification items */}
                {/* {newNotification.map(e => (
                    <li className='list-none transition duration-200 cursor-pointer hover:bg-gray-200 hover:border hover:border-gray-200 hover:rounded-md'>
                        {/* Container for individual notification content */}
                        <div className='relative flex items-center p-2.5'>
                            {/* Displaying the image of the account associated with the notification */}
                            ...
                            <div>
                                {/* Displaying the username of the user who performed the action */}
                                <div><b>{e.user_name}</b>があなたをフォロー中です。</div>
                                {/* Displaying the timestamp or time of the notification */}
                                <div className='text-gray-500 text-md'>{e.created_at}</div>
                            </div>
                        </div>
                    </li>
                ))} */}

                {/* Example of a single notification item (outside the mapping) */}
                {notification.map(e => (
                    <li className='list-none transition duration-200 cursor-pointer hover:bg-gray-200 hover:border hover:border-gray-200 hover:rounded-md'>
                        {/* Container for individual notification content */}
                        <div className='relative flex items-center p-2.5'>
                            ...
                            {/* Details of the notification (using 'notification' singular, assuming it's an object) */}
                            <div>
                                {/* Displaying the username of the user who performed the action */}
                                <div><b>{e.user_name}</b>があなたをフォロー中です。</div>
                                {/* Displaying the timestamp or time of the notification */}
