...
const SignUpPage = () => {
    ...
    //     try {
    //         const response = await http.post(`http://${API_BASE_URL}:8000/api/user/is_registration_allowed`, { id_card: idCard });
    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            // Fetch registration_allowed status
    const response = await http.get(`http://${API_BASE_URL}:8000/api/user`);
    const registrationAllowed = response.data.registration_allowed;

    // Check if registration is allowed
    if (!registrationAllowed) {
        setErrorMessage('Registration is not allowed at the moment.');
        return;
    }
